...
const columns: ColumnDef<Transaction, unknown>[] = [
  ...
  },
  {
    ...
    header: "Branch",
    cell: ({ row }) => (
      <span className="text-sm text-white/70">{row.original.branch.name}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "Type",
    cell: ({ row }) => (
      <TransactionBadge type={row.getValue("type")} />
    ),
  },
  {
    accessorKey: "amount",
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm text-white/60">
        {formatNumber(Number(row.getValue("fee")))}
      </span>
    ),
  },
  {
...
export default function TransactionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["transactions", branchId, type, dateFrom, dateTo],
  ...
  return (
    <>
      <PageHeader title="Transactions" breadcrumb="Admin" />
      ...
      <GlassCard className="mb-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={() => { setBranchId(""); setType(""); setDateFrom(""); setDateTo(""); }}
          >
            Clear
          </Button>
        </div>
